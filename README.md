...
- **Real-Time Detection:** Fast object detection using a lightweight YOLOv8n engine trained over 246 epochs[cite: 1].
- **RESTful Endpoint:** `POST /api/v1/detect` accepts leaf images and returns JSON payloads with class labels, confidence scores, and bounding box coordinates[cite: 1].
...
- **Cross-Species Generalization:** Supports detection across 12 plant diseases and 7 crop species using a dual-dataset mapping approach[cite: 1].

---

## 🛠️ Tech Stack

- **Backend:** FastAPI, Uvicorn, PyTorch, Ultralytics (YOLOv8)[cite: 1]
